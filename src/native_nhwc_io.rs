use {
    rknpu2::{
        ...
        query::{InputOutputNum, NativeNHWCInputAttr, NativeNHWCOutputAttr, TensorAttrView},
    },
    stanza::{
        renderer::Renderer,
        style::Styles,
...
pub fn do_native_nhwc_io(
    ...
    for i in 0..io_num.input_num() {
        let input = rknn_model.query_with_input::<NativeNHWCInputAttr>(i)?;
        table_inputs.push_row(vec![
    ...
    for i in 0..io_num.output_num() {
        let output = rknn_model.query_with_input::<NativeNHWCOutputAttr>(i)?;
        table_outputs.push_row(vec![
    ...
    table_full.push_row(vec!["Native NHWC Inputs", "Native NHWC Outputs"]);
